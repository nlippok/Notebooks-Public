...
* A reliable method to start the tool is to download and launch the Notebook (`notebook.ipynb`) on your computer within a Jupyter Notebook environment.
  1. All the files in the directory above (`*.ipynb`, `setup.png`) are required to run the simulation. To download the entire repository, simply download the `archive.zip` from this directory by clicking on the file and then select **Download** (right side of screen).
  2. Running the Notebook on your computer requires the installation of Jupyter Notebook and Python. We recommend downloading the <a href="https://www.anaconda.com/products/individual" target="_blank">Anaconda</a> package, which is free.  
  3. Open `notebook.ipynb` from inside a Jupyter Notebook environment and initiate from the menu by selecting **Kernel** and then **Restart & Run All**. 
